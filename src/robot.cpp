...
Robot::Robot(uint8_t* map_data, int map_width, int map_height, float start_x, float start_y,
             float start_theta) :
    lidar(map_data, map_width, map_height, 1000),
    state_{start_x, start_y, start_theta},
    goal_{start_x, start_y}
{}
...
void Robot::updatePosition(float dt)
{
    ...
    if (std::abs(theta_error) < 0.5 && std::hypot(goal_.x - state_.x, goal_.y - state_.y) > 10.0f)
    {
...
{
    cv::circle(image, cv::Point2f(state_.x, state_.y), 10, cv::Scalar(0, 255, 0), -1);
    cv::Point2f line_end =
        cv::Point2f(state_.x + 30 * cosf(state_.theta), state_.y + 30 * sinf(state_.theta));
    cv::line(image, cv::Point2f(state_.x, state_.y), line_end, cv::Scalar(0, 255, 0), 2);
}

float* Robot::getLidarData(int& num_points)
{
    lidar.simulateLidar(static_cast<int>(state_.x), static_cast<int>(state_.y), state_.theta,
                        200.0f);
...
void Robot::renderLidar(cv::Mat& image)
{
    ...
    for (int i = 0; i < num_points; ++i)
    {
        ...
        cv::circle(image,
                   cv::Point(static_cast<int>(rx + state_.x), static_cast<int>(ry + state_.y)), 1,
                   cv::Scalar(0, 0, 255), -1);
    }
}

void Robot::renderParticles(cv::Mat& image, Particle* particles, int particle_count, int top)
{
    for (int i = 0; i < top && i < particle_count; ++i)
    {
        const Particle& p = particles[i];
        cv::circle(image, cv::Point2f(p.state.x, p.state.y), 2, cv::Scalar(255, 0, 0), -1);
    }
}

RobotState Robot::getDRState()
{
    return dr_state_;
}
...
{
    cv::Point2f line_end = cv::Point2f(estimated_pose.x + 30 * cosf(estimated_pose.theta),
