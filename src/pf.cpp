...
void ParticleFilter::resample() {
  ...
  int actual_resample_count =
      static_cast<int>(particles_.size() * config_.RESAMPLE_THRESHOLD);
  ...
  for (size_t i = 0; i < particles_.size() - actual_resample_count; i++) {
    ...
    p = particles_[index];
    noise_dx = std::normal_distribution<float>(
        0.0f, config_.POSITION_NOISE_STD_DEV)(std::default_random_engine());
    noise_dy = std::normal_distribution<float>(
        0.0f, config_.POSITION_NOISE_STD_DEV)(std::default_random_engine());
    noise_dtheta = std::normal_distribution<float>(
        0.0f, config_.ANGLE_NOISE_STD_DEV)(std::default_random_engine());
    p.state.x += noise_dx;
    p.state.y += noise_dy;
...
RobotState ParticleFilter::calculateEstimatedPose() {
  ...
  int count = static_cast<int>(particles_.size() *
                               config_.CALCULATE_POSE_POPULATION_FRACTION);
