...
#include <cstdint>
__global__ void traceRaysKernel(uint8_t* map, float* intersections, int width, int height, int x,
                                int y, float theta, int raycount, float maxDistance)
{
...
void GpuLidar2D::simulateLidar(int x, int y, float theta, float max_distance)
{
    ...
    traceRaysKernel<<<n_blocks, 64>>>(d_map_data_, d_intersections_, map_width_, map_height_, x, y,
                                      theta, ray_count_, max_distance);
    CHECK_LAUNCH_ERROR();
