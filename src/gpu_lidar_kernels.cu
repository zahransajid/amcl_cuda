...
#include <cstdint>
__global__ void traceRays(uint8_t *map, float *intersections, int width,
                          int height, int x, int y, float theta, int raycount,
                          float maxDistance) {
...
void GpuLidar2D::simulateLidar(int x, int y, float theta, float max_distance) {
  ...
  traceRays<<<n_blocks, 64>>>(map_data, d_intersections, map_width, map_height,
                              x, y, theta, ray_count, max_distance);
  cudaMemcpy(h_intersections, d_intersections, 2 * ray_count * sizeof(float),
