...
__global__ void traceRays(uint8_t *map, float *intersections, int width, int height, int x, int y, int raycount, float maxDistance)
{
    ...
    if (rayId >= raycount)
        return;
    float theta = (float)rayId * 2.0f * 3.14159265359f / (float)raycount;
    float dx = cosf(theta);
    float dy = sinf(theta);
...
void GpuLidar2D::simulateLidar(int x, int y, float theta, float max_distance)
{
    ...
    traceRays<<<n_blocks, 64>>>(map_data, d_intersections, map_width, map_height, x, y, ray_count, max_distance);
