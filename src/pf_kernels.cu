...
__global__ void correlateParticleKernel(uint8_t* map, int width, int height, int particle_count,
                                        Particle* particle_data, float* lidar_data, int ray_count)
{
...
void ParticleFilter::correlateParticles(float* sensor_data, int ray_count)
{
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_lidar_data_, sensor_data, ray_count_ * 2 * sizeof(float),
                              cudaMemcpyHostToDevice));
    int n_blocks = (config_.MAX_PARTICLE_COUNT + 63) / 64;
    correlateParticleKernel<<<n_blocks, 64>>>(d_map_data_, map_width_, map_height_,
                                              config_.MAX_PARTICLE_COUNT, d_particle_data_,
                                              d_lidar_data_, ray_count_);
    CHECK_LAUNCH_ERROR();
