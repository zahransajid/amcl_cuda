...
{
    CUDA_SAFE_CALL(cudaMalloc(&d_map_data_, width * height * sizeof(uint8_t)));
    CUDA_SAFE_CALL(
        cudaMemcpy(d_map_data_, data, width * height * sizeof(uint8_t), cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMalloc(&d_intersections_, num_rays * sizeof(float) * 2));
...
GpuLidar2D::~GpuLidar2D()
{
    ...
    CUDA_SAFE_CALL(cudaFree(d_intersections_));
}
