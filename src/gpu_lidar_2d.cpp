...
GpuLidar2D::GpuLidar2D(uint8_t *data, int width, int height, int num_rays) {
  cudaMalloc(&map_data_, width * height * sizeof(uint8_t));
  cudaMemcpy(map_data_, data, width * height * sizeof(uint8_t),
             cudaMemcpyHostToDevice);
  cudaMalloc(&d_intersections_, num_rays * sizeof(float) * 2);
  h_intersections_ = new float[num_rays * 2];
  ray_count_ = num_rays;
  map_width_ = width;
  map_height_ = height;
}
