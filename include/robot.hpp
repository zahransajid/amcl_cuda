...
private:
    ...
    float max_motor_velocity = (2 * 3.1415926f * wheel_radius * max_motor_rpm) / 60.0f; // in cm/s

public:
    Robot(uint8_t* map_data, int map_width, int map_height, float start_x, float start_y,
          float start_theta);
    GpuLidar2D lidar;
    ...
    void renderParticles(cv::Mat& image, Particle* particles, int particle_count, int top);
